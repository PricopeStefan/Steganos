...
typedef struct LodePNGColorMode {
    ...
    */
    uint8_t* palette; /*palette in RGBARGBA... order. Must be either 0, or when allocated must have 1024 bytes*/
...
typedef struct LodePNGColorStats {
    ...
    uint8_t palette[1024]; /*Remembers up to the first 256 RGBA colors, in no particular order, only valid when numcolors is valid*/
...
*/
unsigned lodepng_chunk_create(uint8_t** out, size_t* outsize, unsigned length,
