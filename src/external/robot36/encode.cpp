...
int robot36::sstv_encode(char* ppm_path, char* pcm_name)
{
	if (!open_img_read(&img, ppm_path)) {
		fprintf(stderr, "Couldn't load PPM image!");
		return 2;
	}

	if (320 != img->width || 240 != img->height) {
		fprintf(stderr, "PPM image is not 320x240!\n");
		close_img(img);
		return 3;
	}
...
//	fprintf(stderr, "%d %d %d %d %d %d\n", sync_porch_len, porch_len, y_len, uv_len, hor_sync_len, seperator_len);
	...
	if (fabsf(porch_sec * rate - porch_len) > 0.0001)
		std::cout << "this rate will not give accurate (smooth) results.\ntry 40000Hz and resample to " << rate << std::endl;

	hz2rad = (2.0 * M_PI) / rate;
	nco = -I * std::complex<float>{0.7, 0};
	enum { N = 13 };
